...
int add_alias(const char *alias_name, const char *alias_value)
{
	if (alias_count < MAX_ALIASES)
	{
		for (int i = 0; i < alias_count; i++)
		{
			...
			{
				_strcpy(aliases[i].value, alias_value);
				save_alias(alias_name, alias_value);
				return (1);
			}
		}
		_strcpy(aliases[alias_count].name, alias_name);
		_strcpy(aliases[alias_count].value, alias_value);
		alias_count++;
		save_alias(alias_name, alias_value);
		return (1);
